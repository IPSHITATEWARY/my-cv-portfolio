...
const ContactSection = () => {
  ...
  const socials = [
    { label: "LINKEDIN",  sub: "Professional profile", color: "text-violet",  hoverBg: "hover:bg-violet/5",  rule: "bg-violet" },
    { label: "GITHUB",    sub: "Code repositories",    color: "text-cyan",    hoverBg: "hover:bg-cyan/5",    rule: "bg-cyan" },
    { label: "LEETCODE",  sub: "DSA practice",         color: "text-amber",   hoverBg: "hover:bg-amber/5",   rule: "bg-amber" },
  ];

  return (
    <section id="contact" className="bg-card py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div ref={topRef as React.RefObject<HTMLDivElement>}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={item.label} className="flex items-center gap-4">
                  <span className={`font-mono-custom text-[0.6rem] tracking-widest uppercase ${item.labelColor}`}>{item.label}</span>
          ...
          {/* Right: social links */}
          <div ref={linksRef as React.RefObject<HTMLDivElement>} className="space-y-0">
            {socials.map((link, i) => (
              <a
                key={link.label}
                href="#"
              ...
              >
                <div>
                  <div className={`font-display font-black text-foreground text-2xl md:text-3xl group-hover:${link.color} transition-colors duration-200`}>
                    {link.label}
                  </div>
                  <div className="caption-text mt-1">{link.sub}</div>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-6 h-0.5 ${link.rule} opacity-0 group-hover:opacity-100 group-hover:w-10 transition-all duration-300`} />
                  <span className={`text-2xl group-hover:translate-x-2 transition-transform duration-200 ${link.color}`}>→</span>
                </div>
              </a>
            ))}
          </div>
